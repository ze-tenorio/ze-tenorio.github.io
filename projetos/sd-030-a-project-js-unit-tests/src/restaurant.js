...
const createMenu = (obj) => {
  const newObj = { 
    ...
    consumption: [],
    order: (str) => {
      if (obj.food[str] === undefined && obj.drink[str] === undefined) return 'Item indisponível'; 
      newObj.consumption.push(str);
    },
    pay: () => {
      let check = 0;
      for (let order of newObj.consumption) {
        if (obj.food[order] === undefined) check += obj.drink[order];
        if (obj.drink[order] === undefined) check += obj.food[order];
      }
      return check;
    },
  };
