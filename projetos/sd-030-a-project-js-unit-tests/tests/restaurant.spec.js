...
describe('10 - Implemente a função `createMenu`, bem como seus casos de teste', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    const objetoRetornadoAlt = createMenu({ food: { picanha: 49.99 }, drink: {} });
    objetoRetornadoAlt.order('picanha');
    expect(objetoRetornadoAlt.consumption).toContain('picanha');
    expect(objetoRetornadoAlt.order('coxinha')).toBe('Item indisponível');
    objetoRetornadoAlt.order('coxinha');
    expect(objetoRetornadoAlt.consumption).not.toContain('coxinha');
    ...
    const menuRetornado = createMenu({ food: { burguer: 20, fries: 15 }, drink: { coke: 5, water: 2 } })
    menuRetornado.order('fries');
    menuRetornado.order('burguer');
    menuRetornado.order('coke');
    expect(menuRetornado.consumption).toEqual(['fries', 'burguer', 'coke']);
    // 10: Verifique se a função `order` aceita que pedidos repetidos sejam acrescidos a `consumption`.
    menuRetornado.order('fries');
    expect(menuRetornado.consumption).toEqual(['fries', 'burguer', 'coke', 'fries'])
    ...
    expect(menuRetornado.pay()).toBe(55);
